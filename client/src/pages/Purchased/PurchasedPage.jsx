...
import { Plus, Trash2, Search, SlidersHorizontal, X, ChevronDown, Check, MoreVertical, ArrowUpRight, Pencil, PackageMinus, Upload, X as XIcon } from 'lucide-react';
...
/* ─── Filters bar ─── */
const FILTER_KEYS = ['search', 'type', 'quality', 'size', 'condition', 'brand', 'isForResale'];
const EMPTY_FILTERS = { search: '', type: [], quality: [], size: [], condition: [], brand: [], isForResale: '' };
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="space-y-1">
                ...
                <Select value={f.isForResale || '__all__'} onValueChange={(v) => update('isForResale', v === '__all__' ? '' : v)}>
                  <SelectTrigger className="h-8 text-xs"><SelectValue placeholder={t('common.all')} /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__all__">{t('common.all')}</SelectItem>
                    <SelectItem value="true">{t('purchased.status.forSale')}</SelectItem>
                    <SelectItem value="false">{t('purchased.status.notForSale')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
...
export default function PurchasedPage() {
  ...
  const [promoteFiles, setPromoteFiles] = useState([]);
  const [promotePreviews, setPromotePreviews] = useState([]);
  const promoteFileRef = useRef(null);
  ...
  function handlePromoteFileChange(e) {
    const files = Array.from(e.target.files || []);
    if (!files.length) return;
    setPromoteFiles((prev) => [...prev, ...files]);
    files.forEach((f) => {
      const reader = new FileReader();
      reader.onload = (ev) => setPromotePreviews((prev) => [...prev, ev.target.result]);
      reader.readAsDataURL(f);
    });
    e.target.value = '';
  }

  function removePromoteFile(idx) {
    setPromoteFiles((prev) => prev.filter((_, i) => i !== idx));
    setPromotePreviews((prev) => prev.filter((_, i) => i !== idx));
  }
  ...
  async function handlePromote() {
    ...
    try {
      let extraImages = [];
      if (promoteFiles.length > 0) {
        const res = await uploadService.images(promoteFiles);
        extraImages = res.data.data || [];
      }
      await purchaseService.promote(promoteItem._id, { sellPrice: Number(sellPrice) || 0, extraImages, images: promoteItem.images });
  ...
  const columns = [
    {
      ...
      render: (v) => {
        ...
        return img?.url
          ? <img src={img.url} alt="" className="w-10 h-12 object-cover rounded" loading="lazy" />
    ...
    { key: 'season', label: t('purchased.col.season'), render: (v) => v || '—' },
    { key: 'type', label: t('purchased.col.type'), render: (v) => v ? translateJerseyType(v) : '—' },
    { key: 'brand', label: t('jersey.brand'), className: 'hidden md:table-cell', render: (v) => v || '—' },
    { key: 'size', label: t('purchased.col.size'), className: 'hidden sm:table-cell', render: (v) => v || '—' },
    { key: 'condition', label: t('common.status'), className: 'hidden lg:table-cell', render: (v) => v ? translateCondition(v) : '—' },
    {
      key: 'buyPrice',
      label: t('purchased.col.buyPrice'),
      sortable: true,
      render: (v) => v != null ? <span className="font-medium text-[var(--accent)]">{formatCurrency(v)}</span> : '—',
    },
    {
      key: 'purchaseDate',
    ...
    },
    {
      ...
      render: (v, row) => (
        <div className="flex items-center gap-1.5">
          {v
            ? <Badge variant="success">{t('purchased.status.forSale')}</Badge>
            : <Badge variant="secondary">{t('purchased.status.notForSale')}</Badge>}
          {row.linkedJerseyId && (
            <span className="text-[10px] text-[var(--text-muted)]">{t('purchased.status.linked')}</span>
          )}
        </div>
      ),
    },
      ...
      render: (_, row) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            {!row.linkedJerseyId && (
              <DropdownMenuItem
                onClick={() => { setPromoteItem(row); setSellPrice(''); }}
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable columns={columns} data={purchases} loading={loading} emptyText={t('purchased.noItems')} />
      ...
      {/* Promote dialog */}
      <Dialog open={!!promoteItem} onOpenChange={(o) => { if (!o) { setPromoteItem(null); setPromoteFiles([]); setPromotePreviews([]); } }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t('purchased.moveToForSale')}</DialogTitle>
          </DialogHeader>
          {promoteItem && (
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => promoteFileRef.current?.click()}
                  ...
                  >
                    <Upload size={18} />
                    <span className="text-[10px]">{t('purchased.promote.add')}</span>
                  </button>
                  <input ref={promoteFileRef} type="file" accept="image/*" multiple className="hidden" onChange={handlePromoteFileChange} />
                </div>
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => { setPromoteItem(null); setPromoteFiles([]); setPromotePreviews([]); }}>{t('common.cancel')}</Button>
      ...
      <Dialog open={formOpen} onOpenChange={(o) => { if (!o) closeForm(); }}>
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editItem ? t('purchased.editItem') : t('purchased.addItem')}</DialogTitle>
          </DialogHeader>
          <PurchaseForm
